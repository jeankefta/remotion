...
class ExplorimmoSpider(offerSpider):
    name = "explorimmo"
    start_urls = [
        "http://www.explorimmo.com/resultat/annonces.html?transaction=location&location=Paris&priceMax=&type=maison%2Cvilla%2Cferme%2Cmoulin%2Cchalet&type=appartement%2Cchambre&type=atelier%2Cloft%2Cduplex&type=hotel+particulier%2C+propriete%2Cchateau%2Cmanoir&roomMin=&page=1"
    ]
    ...
    def parse_next_page(self, response):
        try:
            for elmt in response.xpath('.//div[@id="vue"]/div[@data-classified-id]'):
                ...
                try:
                    offer.price = elmt.xpath('.//span[@class="price-label"]/text()').extract()[0].strip().replace('\xa0', '')[:-1]
                except Exception:
                    offer.price = None
    ...
    def parse_one_annonce(self, response):
        ...
        offer.area = re.compile(r'(\D+)').sub('', surface[0])
        try:
