...
class MeilleursagentsSpider(offerSpider):
    ...
    start_urls = [
        'http://www.meilleursagents.com/immobilier/recherche/?redirect_url=&view_mode=&sort_mode=&transaction_type=369681778&buyer_search_id=&user_email=&place_ids[]=138724240&place_title=&item_types[]=369681781&item_types[]=369681782&item_area_min=&item_area_max=&budget_min=&budget_max='
    ]
    ...
    def parse_next_page(self, response):
        try:
            for elmt in response.xpath('.//*[@id="body"]/div[@class="container-wide"]/div[contains(@class,"section")]/ul/li[@class="relative"]'):
                ...
                try:
                    html_id = int(re.match(r'.*/annonce-(\d+)/', url).group(1))
                except Exception:
                ...
                offer.area = re.match(r'^.*\s(\d+) m²$', offer.title).group(1)
                try:
                    raw_price = elmt.xpath('.//div[contains(@class,"pull-right")]/div/strong/text()').extract()[0]
                    offer.price = re.match(r'(\d?\s\d+)+', raw_price).group(0).replace(' ', '')
                except Exception:
                    offer.price = None
                offer.address = re.match(
                    r'\s*(.*)\s*',
                    elmt.xpath('.//div[@class="media-body"]//div[@class="pull-left"]/h2[a]/following::div[@class="muted ellipsis"]/text()').extract()[0]
                ).group(1)
    ...
    def parse_one_annonce(self, response):
        offer = super().parse_one_annonce(response)
        try:
